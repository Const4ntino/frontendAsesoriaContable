...
const IngresoModal: React.FC<IngresoModalProps> = ({ open, onClose, ingreso, onSaved, clienteRegimen = "" }) => {
  // Verificar si el cliente es de régimen NRUS
  const isNRUS = clienteRegimen === "NRUS";
  const [form, setForm] = useState<IngresoRequest>({
    idCliente: 0,
    monto: 0,
    montoIgv: 0,
  ...
  useEffect(() => {
    if (ingreso) {
      setForm({
        idCliente: ingreso.cliente.id,
        monto: ingreso.monto,
        montoIgv: ingreso.montoIgv,
        fecha: format(new Date(`${ingreso.fecha}T12:00:00`), "yyyy-MM-dd"),
        descripcion: ingreso.descripcion || "",
        nroComprobante: ingreso.nroComprobante || "",
        urlComprobante: ingreso.urlComprobante || "",
        tipoTributario: ingreso.tipoTributario
      });
    } else {
      setForm({
        idCliente: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const url = isEdit
        ? `http://localhost:8099/api/v1/ingresos/mis-ingresos/${ingreso?.id}`
        : "http://localhost:8099/api/v1/ingresos/mis-ingresos/nuevo";
  ...
  const isFormValid = () => {
    ...
    const basicValidation = form.monto > 0 && form.fecha;
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Editar Ingreso" : "Registrar Nuevo Ingreso"}</DialogTitle>
          <DialogDescription>
            {isEdit
